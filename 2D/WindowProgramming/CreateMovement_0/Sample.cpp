#include "TCore.h"
#include "TDxObject.h"

using namespace std;
class Sample : public TCore 
{
	TDxObject objScreen; // »ï°¢Çü
	vector<TDxObject> objList;
	TDxObject hero;
public:
	void Init() override
	{
		RECT rtBk = { 0, 0, g_xClientSize, g_yClientSize }; // left, top, right, bottom
		objScreen.Create(m_pd3dDevice.Get(), m_pContext, rtBk, L"bk.jpg");

		objList.resize(3);
		// 1024, 768
		objList[0].Create(m_pd3dDevice.Get(), m_pContext, { 0, 0, 100, 100 }, L"sun.jpg");
		objList[1].Create(m_pd3dDevice.Get(), m_pContext, { 924, 0, 1024, 100 }, L"planet1.jpg");
		objList[2].Create(m_pd3dDevice.Get(), m_pContext, { 924, 668, 1024, 768 }, L"earth.jpg");
		hero.Create(m_pd3dDevice.Get(), m_pContext, { 462, 334, 562, 434 }, L"spaceship.jpg");
	}

	void Frame() override
	{
		float fSpeed = 100.0f * g_fSecondPerFrame;
		if (m_Input.KeyCheck('W') == KEY_HOLD)
		{
			hero.Move(0.0f, -fSpeed);
		}
		if (m_Input.KeyCheck('S') == KEY_HOLD)
		{
			hero.Move(0.0f, fSpeed);
		}
		if (m_Input.KeyCheck('A') == KEY_HOLD)
		{
			hero.Move(-fSpeed, 0);
		}
		if (m_Input.KeyCheck('D') == KEY_HOLD)
		{
			hero.Move(fSpeed, 0);
		}
		hero.Render(m_pContext);
	}
	void Render() override
	{
		objScreen.Render(m_pContext);
		/*for (auto& obj : objList)
		{
			obj.Render(m_pContext);
		}*/
		//auto LamdaRender = [&](auto& obj) { obj.Render(m_pContext); };
		for_each(begin(objList), end(objList), [&](auto& obj)
			{
				obj.Render(m_pContext);
			});

		hero.Render(m_pContext);
	}
	void   Release() override
	{
		objScreen.Release();
		for (auto& obj : objList)
		{
			obj.Release();
		}
		hero.Release();
	}
};

T_GAME_START(1024, 768);